...
function installTileFallback(map, primary, overlay){
  ...
  function switchToFallback(){
    ...
    const fallback1 = L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/dark_all/{z}/{x}/{y}{r}.png',
      { subdomains:'abcd', maxZoom:8, attribution:'&copy; OpenStreetMap &copy; CARTO' }).addTo(map)
    let switched2=false
    fallback1.on('tileerror', ()=>{
      if(switched2) return
      switched2=true
      try{ if(map.hasLayer(fallback1)) map.removeLayer(fallback1) }catch(e){}
  ...
  setTimeout(()=>{
    const loaded=Object.keys(primary._tiles||{}).length + (overlay? Object.keys(overlay._tiles||{}).length:0)
